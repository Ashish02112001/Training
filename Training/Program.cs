...
namespace Training {
   public class Wordle {
      ...
      public Wordle (Tuple<string, List<string>> tuple) {
         mWord = tuple.Item1;
         mList = tuple.Item2;
      ...
      public void Run () {
         if (mList is null) {
            ...
            foreach (string word in mList) {
               ...
               if (ind == 5) {
                  UpdateGameState (ConsoleKey.Enter);
                  DisplayBoard ();
                  ind = 0;
               }
            }
      ...
      void DisplayBoard () {
         ...
         Put (KBDX, KBDY - 2, Gray, new string ('_', 31));
         ...
         if (mList is null) Put (WindowWidth / 2 - 10, RESULTY + 1, Yellow, error);
         else Put (15, RESULTY + 1, Yellow, error);
      }
      int GRIDX = 3, GRIDY = 1;
      int KBDX = 3, KBDY = 14;
      ...
      void UpdateGameState (ConsoleKey info) {
         ...
         if (info is ConsoleKey.Enter && mX == 5) {
            ...
            // First, if the current word is not in the dictionary, don't 
            // accept it
      ...
      void PrintResult () {
         ...
         if (mList is null) {
            ...
            if (mSucceeded)
               ...
               Put (15, RESULTY + 2, Yellow, $"Sorry - the word was {mWord}");
            Put (15, RESULTY + 4, White, "Press any key to quit");
         }
      }
      int RESULTY = 18;
      ...
      void Put (int x, int y, ConsoleColor color, object data) {
         ...
         using (StreamWriter sw = new (mPath, true)) {
            ...
            switch (color) {
               ...
               case Blue: SwWrite ("[" + ch + "]"); break;
               case DarkGray: SwWrite ("(" + ch + ")"); break;
               default: SwWrite (ch); break;
            }
            if (x is 15 or 33) sw.WriteLine ();
            else if (x is 23 && data is 'Z') {
               sw.WriteLine ();
               SwWrite (new string ('_', 33));
               sw.WriteLine ();
            }
            void SwWrite (string d) {
               if (!string.IsNullOrWhiteSpace (d) && y != 12 && y != 18) {
                  if (d.IndexOfAny ("{[(".ToCharArray ()) == -1) sw.Write ($"  {d}  ");
                  else sw.Write ($" {d} ");
               }
            }
         }
